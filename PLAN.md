...
### Experiment 1 — Hebbian Query-Value Gate
...
**Change**: Expand `ve_gate` from `Linear(32, K)` → `Linear(32+K, K)` where K=n_kv_head. Augment gate input with per-head cosine similarity between pre-RoPE query and value embedding. Zero-initialize so step-0 behavior is identical to baseline.

**Risk**: Low. Gradient through cosine similarity is bounded. Parameter count unchanged (same tensor count).

**Expected effect**: Moderate improvement. Better value residual gating should help early in training and at convergence.
